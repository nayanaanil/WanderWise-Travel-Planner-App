...
export default function ProfilePage() {
  ...
  const handleScanTravelStyle = async () => {
    ...
    try {
      ...
      const formattedResult = `Inferred From: ${scanData.inferredFrom}\nConfidence: ${scanData.confidence}\n\nTraits:\n${scanData.traits.map((t: any) => `  - ${t.name}: ${t.value} (${t.confidence} confidence)`).join('\n')}\n\nEvidence Snippets:\n${scanData.evidenceSnippets.map((s: any) => `  - [${s.trait}] "${s.text}"`).join('\n')}`;
          ...
          <div className="flex-1">
            ...
            {extractedText && (
              <div className="mt-6 p-4 bg-white border border-gray-200 rounded-xl">
                <h3 className="text-sm font-semibold text-gray-900 mb-2">Formatted Results</h3>
